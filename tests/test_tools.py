...
class TestToolSet:
    ...
    def test_tool_replacement_direct(self, basic_toolset):
        ...
        assert basic_toolset.get(tool.__name__)() == "second"

    def test_tool_replacement_in_list(self, basic_toolset):
        """Test replacing tools when adding from a list"""
        def tool1(): return "original1"
        def tool2(): return "original2"
        ...
        def new_tool1(): return "replaced1"
        def new_tool2(): return "replaced2"
        ...
        assert len(basic_toolset.tools) == original_length
        assert basic_toolset.get(tool1.__name__)() == "replaced1"
        assert basic_toolset.get(tool2.__name__)() == "replaced2"

    def test_tool_replacement_in_dict(self, basic_toolset):
    ...
    def test_tool_replacement_nested_structure(self, basic_toolset):
        """Test replacing tools in nested structure"""
        def tool1(): return "original1"
        def tool2(): return "original2"
        def tool3(): return "original3"
        ...
        def new_tool1(): return "replaced1"
        def new_tool2(): return "replaced2"
        def new_tool3(): return "replaced3"
        new_tool1.__name__ = tool1.__name__
        new_tool2.__name__ = tool2.__name__
        ...
        # Replace tools
        basic_toolset.add({
            "new_level1": {"inner": [new_tool1, new_tool2]},
            "new_level2": new_tool3
        })
        ...
        assert basic_toolset.get(tool1.__name__)() == "replaced1"
        assert basic_toolset.get(tool2.__name__)() == "replaced2"
        assert basic_toolset.get(tool3.__name__)() == "replaced3"
    ...
    def test_duplicate_tool_different_reference(self, basic_toolset):
        ...
        basic_toolset.add(tool)
        assert len(basic_toolset.tools) == original_length  # Length should not change
        assert basic_toolset.get("tool")() == "second"  # Should use latest version
